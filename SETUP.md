...
**Installation** : sur PC (Chrome/Edge), un bandeau avec bouton "Installer" apparaît automatiquement quand le navigateur détecte que l'app est installable. Sur iPhone, Safari ne permet pas ce bouton automatique — le bandeau affiche donc l'instruction (Partager → Sur l'écran d'accueil) à la place. Détection réelle par capacité disponible, pas par nom de navigateur.
...
Le navigateur doit charger la page depuis une vraie URL https (pas juste ouvrir le fichier localement) pour que la connexion Google fonctionne — c'est une exigence de Google. Déposer tous les fichiers ensemble, dans le même dossier. Options simples :
...
Différent des erreurs ci-dessus : ici la connexion Google a réussi, mais l'API elle-même (Gmail ou Drive selon le mode utilisé) n'est pas activée sur le projet Google Cloud. Le message d'erreur contient un lien direct vers la bonne page pour l'activer (**Enable**) — vérifier aussi qu'il s'agit bien du **même projet** que celui où le Client ID a été créé.

### Blocage CORS / le transfert Gmail échouait systématiquement ("Load failed") ET augmentait la taille de la boîte (corrigé)

Trois symptômes différents d'une même cause :

Une version intermédiaire de l'app envoyait les messages volumineux via le point d'entrée "upload" de l'API Gmail (`/upload/gmail/v1/...`) plutôt que le point d'entrée standard, sur la base d'une limite de taille supposée de 1 Mo qui n'existe pas réellement (confusion avec une limite de longueur de ligne de commande d'un outil sans rapport). **Ce point d'entrée upload n'accepte pas les requêtes directes depuis un navigateur (CORS)** — contrairement au point d'entrée standard, lui bien prévu pour ça. Résultat : sur mobile Safari, la requête échouait avant même de partir ("Load failed") ; sur PC, le comportement peut différer selon le navigateur mais reste au mieux non fiable.

L'app revient au point d'entrée standard (`/gmail/v1/users/me/messages/send`), confirmé compatible CORS.

**Cause séparée de l'augmentation de taille**, cumulée avec ce qui précède : Gmail enregistre **systématiquement** une copie de tout message envoyé dans "Envois" — y compris via l'API, sans aucun moyen de le désactiver. Un transfert réussi ajoutait donc une copie à la boîte source en plus de l'original. L'app supprime maintenant aussi cette copie juste après l'avoir créée (même règle corbeille/définitif que le reste) ; un échec sur cette étape spécifique est mineur (l'original est déjà traité) et signalé à part sur l'écran final plutôt que noyé dans le compte d'erreurs.

Un email individuel qui dépasse 25 Mo (limite d'émission de Gmail lui-même, tous canaux confondus) est téléchargé sur l'appareil (fichier `.eml`) au lieu d'être transféré ; la suppression Gmail se fait ensuite normalement.

**Si un blocage CORS apparaît malgré tout** (console développeur PC : message explicite "blocked by CORS policy" ou similaire) sur un appel autre que celui corrigé ci-dessus : vérifier qu'aucune extension navigateur (bloqueur de pub/tracking, VPN) n'intercepte les requêtes vers `googleapis.com`, et que l'origine de la page correspond bien à celle enregistrée (voir l'erreur 401 ci-dessus — un mauvais enregistrement d'origine peut aussi se manifester comme une erreur CORS générique selon le navigateur).

### Le traitement échoue en bloc sur un gros lot ("Load failed" ou erreur réseau similaire, après ce correctif)

Cause la plus probable désormais : l'app a été mise en arrière-plan pendant le traitement (écran verrouillé, changement d'appli) — Safari coupe alors les requêtes réseau en cours. L'app attend maintenant que la page redevienne visible avant de lancer l'élément suivant ("En pause" affiché entre-temps). Garder l'app ouverte et l'écran allumé jusqu'à la fin du traitement reste la meilleure prévention.

### La page se ferme / revient en arrière pendant un traitement volumineux

Cause probable : mémoire saturée. Transférer un email ou télécharger un fichier Drive passe par la mémoire du navigateur (pas de flux direct vers le disque, limitation de Safari) — un lot de plusieurs centaines de Mo peut dépasser ce que Safari autorise sur mobile. L'app affiche un avertissement (jauge + confirmation) au-delà de 150 Mo sélectionnés, et marque une pause toutes les 5 éléments traités ; en cas de blocage malgré tout, décoche une partie de la sélection et traite par groupes plus petits.
...
- **Échec partiel** : le transfert/téléchargement a réussi mais la suppression a échoué ensuite. L'app retente automatiquement une fois avant d'abandonner, et l'écran final signale distinctement ce cas plutôt que de le compter comme une simple erreur générique.
- **Délai d'indexation Gmail** : après une mise à la corbeille, l'index de recherche Gmail peut mettre quelques instants à se mettre à jour.

## Différences avec la version Python

- Pas de `credentials.json` ni de jeton à gérer : la connexion se refait à chaque session (jeton valable ~1h).
...
Le Client ID n'est pas un secret : personne ne peut l'utiliser pour accéder au compte sans passer par sa propre connexion Google. Il est stocké dans le stockage local du navigateur sur l'appareil utilisé.

Le scope Drive utilisé (`.../auth/drive`) donne un accès complet au Drive, nécessaire pour retrouver et supprimer n'importe quel fichier volumineux existant.

## Repère de version

L'app affiche en bas de l'écran un horodatage ("Version AAAA-MM-JJ HH:MM"). Utile pour vérifier après un nouveau dépôt de fichiers que la version testée est bien la dernière, plutôt qu'une version mise en cache par le navigateur ou l'écran d'accueil.
